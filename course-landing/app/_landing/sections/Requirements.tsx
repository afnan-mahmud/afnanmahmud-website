        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {ITEMS.map((item, i) => (
            <Reveal key={item.label} delay={i * 60}>
              <div className="flex h-full items-center gap-4 rounded-2xl border border-[var(--line)] bg-white p-5">
                <div
                ...
                <span className="text-sm font-bold leading-snug text-[var(--ink)]">{item.label}</span>
              </div>
            </Reveal>
          ))}
        </div>
