...
**Device-limited sessions (anti account-sharing).** Each **student** account may hold at most one mobile + one desktop session (tablet = mobile; admins exempt). Because JWT is stateless and the edge middleware can't reach the DB, enforcement is server-side at Node boundaries. On login, `authorize` calls `registerDeviceSession` (`lib/auth-device.ts`) which overwrites that device-class's slot in `User.deviceSessions` and returns a unique `sessionId` carried in the JWT (`token.sessionId` → `session.user.sessionId`). A new same-class login removes the previous device's `sessionId`. `isSessionActive(userId, role, sessionId)` re-checks it in the server `app/(dashboard)/layout.tsx` (which renders the client `DashboardShell`) and in `POST /api/video/otp`; a kicked session is redirected to `/auth/otp?reason=other_device`, where the page clears the stale cookie via `signOut`. Device class comes from the User-Agent via `lib/device.ts`. Tokens issued before this feature have no `sessionId` and are treated as inactive (one-time re-login).
...
### Video playback (VdoCipher, `lib/vdocipher.ts`)
Course videos are DRM-protected (Widevine/FairPlay) on VdoCipher, **not** YouTube. Lesson `videoId` and `Course.previewVideoId` store **VdoCipher Video IDs** (paste the ID from the VdoCipher dashboard into the admin lesson/media fields — no in-admin upload). Playback flow: the client `VdoPlayer` (`components/VdoPlayer.tsx`) POSTs the `videoId` to the auth-gated `POST /api/video/otp`, which resolves it against published-course data, enforces ownership (paid lessons require the course in `User.purchasedCourses`; `isPreview` lessons and `previewVideoId` are public), then calls `getOtp` to mint a short-lived OTP with a moving `rtext` **watermark = the buyer's phone number** (built by `buildWatermark`, derived server-side from the session — never trusted from the client). The player loads `https://player.vdocipher.com/v2/?otp=…&playbackInfo=…`. The OTP API (`POST https://dev.vdocipher.com/api/videos/{videoId}/otp`, header `Authorization: Apisecret VDOCIPHER_API_SECRET`) is **server-only**. When `VDOCIPHER_API_SECRET` is unset the OTP route returns `503` and the player shows a "not configured" state — it never crashes.
